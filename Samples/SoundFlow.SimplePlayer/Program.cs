...
internal static class Program
{
        ...
        SampleFormat sampleFormat = SampleFormat.F32, int channels = 2)
    {
        ...
        else if ((_audioEngine.Capability & capability) != capability || _audioEngine.SampleRate != sampleRate || _audioEngine.SampleFormat != sampleFormat || AudioEngine.Channels != channels)
        {
            _audioEngine.Dispose();
            _audioEngine = new MiniAudioEngine(sampleRate, capability, sampleFormat, channels);
        }
    ...
    private static void PlaybackControls(ISoundPlayer player)
    {
        ...
        timer.Elapsed += (_, _) =>
        {
            if (player.State != PlaybackState.Stopped)
            {
                ...
                Console.Write($"Time: {(int)player.Time / 60}:{player.Time % 60:00} / Duration: {(int)player.Duration / 60}:{player.Duration % 60:00}        ");
            }
            else
            {
                timer.Stop();
    ...
    private static void PlayAudioFromFile()
    {
        ...
        PlayAudio(new StreamDataProvider(new FileStream(filePath, FileMode.Open, FileAccess.Read)), isSurround, player =>
        {
            if (isSurround && player is SurroundPlayer surroundPlayer)
            {
                ...
                surroundPlayer.ListenerPosition = new Vector2(0.5f, 0f);
                surroundPlayer.SpeakerConfig = SurroundPlayer.SpeakerConfiguration.Surround71;
            }
            else if (!isSurround && player is SurroundPlayer)
            {
    ...
    private static void PlayAudioWithEqualizer()
    {
        ...
        if (int.TryParse(Console.ReadLine(), out var presetChoice) && presetChoice > 0 && presetChoice <= presetNames.Count)
        {
