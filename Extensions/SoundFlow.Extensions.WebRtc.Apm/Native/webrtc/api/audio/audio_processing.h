...
// C-compatible handle types
#ifdef __cplusplus
extern "C" {
#endif

// Opaque pointer types for C interface
typedef struct webrtc_apm webrtc_apm;
typedef struct webrtc_apm_config webrtc_apm_config;
typedef struct webrtc_stream_config webrtc_stream_config;
typedef struct webrtc_processing_config webrtc_processing_config;

// Error codes
typedef enum {
    WEBRTC_APM_NO_ERROR = 0,
    WEBRTC_APM_UNSPECIFIED_ERROR = -1,
    WEBRTC_APM_CREATION_FAILED = -2,
    WEBRTC_APM_BAD_PARAMETER = -6,
    WEBRTC_APM_BAD_SAMPLE_RATE = -7,
    WEBRTC_APM_BAD_DATA_LENGTH = -8,
    WEBRTC_APM_BAD_NUM_CHANNELS = -9
} webrtc_apm_error;
...
typedef enum {
    WEBRTC_APM_GC_ADAPTIVE_ANALOG,
    WEBRTC_APM_GC_ADAPTIVE_DIGITAL,
    WEBRTC_APM_GC_FIXED_DIGITAL
} webrtc_apm_gc_mode;

// Downmix methods
typedef enum {
    WEBRTC_APM_DOWNMIX_AVERAGE_CHANNELS,
    WEBRTC_APM_DOWNMIX_USE_FIRST_CHANNEL
} webrtc_apm_downmix_method;

// Runtime setting types
typedef enum {
    WEBRTC_APM_RUNTIME_CAPTURE_PRE_GAIN,
    WEBRTC_APM_RUNTIME_CAPTURE_COMPRESSION_GAIN,
    WEBRTC_APM_RUNTIME_CAPTURE_FIXED_POST_GAIN,
    WEBRTC_APM_RUNTIME_PLAYOUT_VOLUME_CHANGE,
    WEBRTC_APM_RUNTIME_CUSTOM_RENDER_SETTING,
    WEBRTC_APM_RUNTIME_PLAYOUT_AUDIO_DEVICE_CHANGE,
    WEBRTC_APM_RUNTIME_CAPTURE_POST_GAIN,
    WEBRTC_APM_RUNTIME_CAPTURE_OUTPUT_USED
} webrtc_apm_runtime_setting_type;
...
RTC_EXPORT void webrtc_apm_config_set_noise_suppression(webrtc_apm_config* config, int enabled, webrtc_apm_ns_level level);
RTC_EXPORT void webrtc_apm_config_set_gain_controller1(webrtc_apm_config* config, int enabled, webrtc_apm_gc_mode mode,
                                           int target_level_dbfs, int compression_gain_db, int enable_limiter);
RTC_EXPORT void webrtc_apm_config_set_gain_controller2(webrtc_apm_config* config, int enabled);
RTC_EXPORT void webrtc_apm_config_set_high_pass_filter(webrtc_apm_config* config, int enabled);
RTC_EXPORT void webrtc_apm_config_set_pre_amplifier(webrtc_apm_config* config, int enabled, float fixed_gain_factor);
RTC_EXPORT void webrtc_apm_config_set_pipeline(webrtc_apm_config* config, int max_internal_rate,
...
RTC_EXPORT int webrtc_apm_proc_split_sample_rate_hz(const webrtc_apm* apm);
RTC_EXPORT size_t webrtc_apm_num_input_channels(const webrtc_apm* apm);
RTC_EXPORT size_t webrtc_apm_num_proc_channels(const webrtc_apm* apm);
RTC_EXPORT size_t webrtc_apm_num_output_channels(const webrtc_apm* apm);
