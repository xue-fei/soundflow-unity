...
namespace SoundFlow.Extensions.WebRtc.Apm.Modifiers
{
    ...
    public sealed class WebRtcApmModifier : SoundModifier, IDisposable
    {
        ...
        private readonly object _apmLock = new object();
        ...
        private IntPtr[]? _dummyReverseOutputChannelPtrs; // For AEC farend processing
        private IntPtr _dummyReverseOutputChannelArrayPtr = IntPtr.Zero;
        private GCHandle _dummyReverseOutputChannelArrayHandle;

        private readonly Queue<float> _inputRingBuffer = new Queue<float>();
        private readonly Queue<float> _outputRingBuffer = new Queue<float>();
        private readonly Queue<float> _farendInputRingBuffer = new Queue<float>();
        ...
        private void InitializeApmAndFeatures()
        {
            ...
            lock (_apmLock)
            {
                ...
                try
                {
                    ...
                    _inputChannelPtrs = new IntPtr[_numChannels];
                    _outputChannelPtrs = new IntPtr[_numChannels];
                    _farendChannelPtrs = new IntPtr[_numChannels];
                    _dummyReverseOutputChannelPtrs = new IntPtr[_numChannels];
        ...
        public override void Process(Span<float> buffer) // Near-end processing
        {
            if (!Enabled || !_isApmSuccessfullyInitialized || _apm == null || _apmConfig == null ||
                ...
                _deinterleavedOutputApmFrame == null || _inputChannelArrayPtr == IntPtr.Zero ||
                _outputChannelArrayPtr == IntPtr.Zero || buffer.Length == 0)
                return;
        ...
        private void HandleAudioEngineProcessedForAec(Span<float> samples, Capability capability) // Far-end processing
        {
            if (capability != Capability.Playback || !Enabled || !_isApmSuccessfullyInitialized || _apm == null ||
                ...
                _reverseOutputStreamConfig == null || _farendChannelArrayPtr == IntPtr.Zero ||
        ...
        private void DisposeApmNativeResources()
        {
            lock (_apmLock)
            {
                ...
                // Free HGlobal memory
                Action<IntPtr[]?> freePtrArray = arr =>
                {
