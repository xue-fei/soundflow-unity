...
namespace SoundFlow.Abstracts
{
    ...
    public abstract class AudioEngine : IDisposable
    {
        ...
        private readonly ManualResetEvent _audioThreadStarted = new ManualResetEvent(false);
        private readonly ManualResetEvent _stopAudioThread = new ManualResetEvent(false);
        ...
        protected void ProcessGraph(IntPtr output, int length)
        {
        ...
        protected void ProcessAudioInput(IntPtr input, int length)
        {
        ...
        /// </summary>
        private void ProcessAudio(IntPtr deviceBufferPtr, int length, ProcessingDirection direction)
        {
        ...
        private void ConvertToDeviceFormat(Span<float> source, IntPtr destination, int length)
        {
            switch (SampleFormat)
            {
        ...
        private void ConvertFromDeviceFormat(IntPtr source, Span<float> destination, int length)
        {
            switch (SampleFormat)
            {
        ...
        private static void ConvertFloatTo<T>(Span<float> floatBuffer, IntPtr output, int length) where T : unmanaged
        {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ConvertFrom<T>(IntPtr input, Span<float> floatBuffer, int length) where T : unmanaged
        {
            if (typeof(T) == typeof(byte))
            {
        ...
        /// </summary>
        private static void ConvertFloatToS24(Span<float> floatBuffer, IntPtr output, int length)
        {
        ...
        /// </summary>
        private static void ConvertFromS24(IntPtr input, Span<float> floatBuffer, int length)
        {
